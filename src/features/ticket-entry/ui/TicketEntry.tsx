...
import { useUpdateTicketMutation } from '../model'
...
export function TicketEntry({ ticket }: Props) {
  ...
  return (
    <div className="w-full container mx-auto flex flex-col gap-y-2">
      ...
      <TicketEntryHeader ticket={ticket} status={draft.status} onStatusChange={(v) => updateDraft('status', v)} />
      <div className="w-full flex justify-between gap-x-4 ">
        <TicketEntryContent ticket={ticket} onStatusChange={(v) => updateDraft('status', v)} />
        <TicketEntryDetails
          ticket={ticket}
